...
function PlanningPanel({ project, categories, weeklySettings, onPlanGenerated }: PlanningPanelProps) {
  ...
  const handleValidation = () => {
    const validationErrors = validatePlanningData(project, categories, weeklySettings);
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">カテゴリーと設定に基づいてタスク計画を自動生成します</p>
        </div>
        <button
      ...
      {errors.length > 0 && (
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-md">
          <h4 className="text-sm font-medium text-red-800 mb-2">設定に問題があります:</h4>
                ...
                <div key={category.id} className="flex justify-between items-center py-2 px-3 bg-gray-50 rounded">
                  <div>
                    ...
                    <span className="ml-2 text-sm text-gray-600">
                      ({category.valueRange.min}-{category.valueRange.max}, 単位{category.minUnit})
                    </span>
                  </div>
                  <div className="text-sm text-gray-600">
                    ...
                    {category.deadline && (
                      <span className="ml-2 text-orange-600">
                        期限: {new Date(category.deadline).toLocaleDateString('ja-JP')}
                      </span>
                    )}
                  </div>
                </div>
      ...
      <div className="flex justify-center">
        <button
          ...
          disabled={isGenerating || categories.length === 0 || errors.length > 0}
      ...
      {showPreview && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-md">
          <div className="flex items-center">
            ...
            <span className="text-green-800 font-medium">計画が生成されました！</span>
          </div>
