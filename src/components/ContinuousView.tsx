...
const ContinuousView = forwardRef<ContinuousViewRef, ContinuousViewProps>(
  ({ taskBlocks, categories, onToggleTaskCompletion, onMoveTaskBlock, projectStartDate, projectEndDate }, ref) => {
        ...
        <div className="space-y-1">
          {getDatesInRange.map((date) => {
            ...
            return (
              ...
              >
                {/* 日付部分 */}
                <div className={`flex-shrink-0 w-32 p-3 text-center border-r border-gray-200 ${
                  dateInfo.isWeekend ? 'bg-red-50' : 'bg-gray-50'
                }`}>
                  <div className={`text-sm font-medium ${
                    dateInfo.isToday ? 'text-blue-700' : 
                    dateInfo.isWeekend ? 'text-red-600' : 'text-gray-700'
                  }`}>
                    {dateInfo.dateString}
                  </div>
                  <div className={`text-xs ${
                    dateInfo.isToday ? 'text-blue-600' : 
                    dateInfo.isWeekend ? 'text-red-500' : 'text-gray-500'
                  }`}>
                    ({dateInfo.dayName})
                  </div>
                  {dateInfo.isToday && (
