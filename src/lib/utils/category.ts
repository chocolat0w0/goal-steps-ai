...
export function calculateProgress(completedBlocksCount: number, totalUnits: number) {
  const percentage = totalUnits > 0 ? Math.round((completedBlocksCount / totalUnits) * 100) : 0;
