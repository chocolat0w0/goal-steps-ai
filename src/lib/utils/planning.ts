...
export function getDayKeyFromDayOfWeek(
  dayOfWeek: number
): keyof Omit<WeeklySettings, 'projectId' | 'id'> | null {
  const dayMap = {
    0: 'sunday', // Sunday
    1: 'monday', // Monday
    2: 'tuesday', // Tuesday
...
export function calculateDailyCapacities(
  ...
  const totalWeightedDays = dailyMultipliers.reduce(
  ...
  const targetRatios = dailyMultipliers.map(
    (multiplier) => multiplier / totalWeightedDays
  );
