...
export function generatePlan(
  ...
  const completedTaskBlocks = existingTaskBlocks.filter(
    (t) => t.projectId === project.id && t.completed
  );
  const nonProjectTaskBlocks = existingTaskBlocks.filter(
...
function calculateCompletedAmountsByCategory(
  ...
  completedTaskBlocks.forEach((taskBlock) => {
    ...
    completedAmounts.set(
      taskBlock.categoryId,
      currentAmount + taskBlock.amount
    );
  });
...
export function createOptimizedTaskBlocks(
  ...
  const totalRemainingUnits = remainingUnits.reduce(
    (sum, units) => sum + units,
    0
  );
  ...
  const categoryWeights = remainingUnits.map(
    (units) => units / totalRemainingUnits
  );
...
function calculateDailyAllocations(
  ...
  return dailyCapacities.map((capacity) => {
    ...
    if (maxCategoriesPerDay && categories.length > maxCategoriesPerDay) {
      allocations = constrainCategoriesPerDay(
        allocations,
        categoryWeights,
        maxCategoriesPerDay,
        capacity
    ...
    for (let i = 0; i < remaining; i++) {
      ...
      for (const { index } of sortedIndices) {
        if (
          allocations[index] > 0 ||
          !maxCategoriesPerDay ||
          allocations.filter((a) => a > 0).length < maxCategoriesPerDay
        ) {
          allocations[index]++;
          break;
        }
...
function constrainCategoriesPerDay(
  ...
  const totalSelectedWeight = topCategories.reduce(
    (sum, cat) => sum + cat.weight,
    0
  );

  topCategories.forEach(({ index, weight }) => {
    newAllocations[index] = Math.floor(
      (weight / totalSelectedWeight) * totalCapacity
    );
  });
